...
$(document).ready(function(){
    ...
    // Отправка писем
    // Технология Ajax - действия с сервером происходят без перезагрузки страницы
    $('form').submit(function(e) {
        ...
        $.ajax({
            ...
            data: $(this).serialize()
    // $('form') получаем все формы
    // .submit() - подтверждаются формы, когда прошли все валидации, формв отправляется
    // e - аргумент event
    // в стандартном поведении при нажатии кнопки отправить форму, страница перезагружается
    // preventDefault() - отменяем стандартное поведение браузера
    // тк отключили стандартное поведение, нужно отправить форму
    // метод ajax в библиотеке jQuery
    // в ajax прописываем объект
    // type: '' - указываем данные получаем или отдаём серверу
    // после указываем какой обработчик должен обрабатывать всю операцию
    // третья часть - данные которые нужно отправить на сервер
    // .serialize() - подготовка данных перед отправкой
    // серверная часть принимает его в другом формате
        }).done(function() {
            $(this).find("input").val("");
            // .done - если всё успешно отправлено, вызываем функцию
            // .find - находим 'input'
            // .val('') - велью устанавливаем в пустую строку
            // после отправки формы очистим все интупы
            $('#consultation, #order').fadeOut();
            $('.overlay, #thanks').fadeIn('slow');

            $('form').trigger('reset');
            // .trigger('reset') - все формы должны очиститься
        });
        return false;
    });
    ...
    $(window).scroll(function() {
        // window - всё окно браузера
        // .scroll(function() - во время скролла будет происходит функция
    ...
    $("a[href=#up]").click(function(){
        // $("a получаем все ссылки по определённым параметрам
        // [] - означает, что будет какой-то атрибут
        const _href = $(this).attr("href");
        // в переменную помещаем атрибут attr("href")
        // дальше идёт анимация на jQuery
